...
Top:                    ; Our read loop.
    ...
    OR   R5, R3, R4     ; BLUE = not (b1 or b2), still need 'not'.
    XOR  R5, R5, #1     ; BLUE = not (b1 or b2), here we invert that bit.

    LSL  R5, R5, #1     ; Need to pack RED and GREEN in lower bits.
    AND  R6, R3, R4     ; GREEN = b1 and b2.
    OR   R5, R5, R6     ; R5 = (BLUE << 1) | GREEN

    LSL  R5, R5, #1     ; Still need to pack RED
    XOR  R6, R3, R4     ; RED = b1 xor b2
    OR   R5, R5, R6     ; R5 = (((BLUE << 1) | GREEN) << 1) | RED
